...
interface BazucandoBannerProps {
  onClick?: () => void;
}

export default function BazucandoBanner({ onClick }: BazucandoBannerProps) {
  return (
    <div
      onClick={onClick}
      role="button"
    ...
    >
      <div className="flex items-center gap-6 transition-transform duration-500 group-hover:scale-105">
        ...
        <div>
          <h2 className="text-4xl font-black uppercase tracking-[0.2em] text-white md:text-5xl">BAZUCANDO</h2>
          <p className="mt-2 text-center text-[10px] font-bold uppercase tracking-[0.24em] text-emerald-200/80">OSINT Brasil / bot de consultas</p>
        </div>
        <Crosshair className="h-10 w-10 text-emerald-300" />
      </div>
    </div>
  );
}
